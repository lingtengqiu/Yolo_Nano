...
def ap_per_class(tp, conf, pred_cls, target_cls,tiou):
    ...
    ap, p, r = [], [], []
    for c in tqdm.tqdm(unique_classes, desc="Computing AP@{:.2f}".format(tiou)):
        i = pred_cls == c
...
def get_batch_statistics(outputs, targets, iou_threshold):
    ...
    for sample_i in range(len(outputs)):
        ...
        true_positives = np.zeros((len(iou_threshold),pred_boxes.shape[0]))
        annotations = targets[targets[:, 0] == sample_i][:, 1:]
        target_labels = annotations[:, 0] if len(annotations) else []
        if len(annotations):
            detected_boxes = [[]for i in range(len(iou_threshold))]
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                for index,_iou in enumerate(iou_threshold):
                    if iou >= _iou and box_index not in detected_boxes[index]:
                        true_positives[index,pred_i] = 1
                        detected_boxes[index] += [box_index]
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres,img_scores = None,gt_mix_index = None):
    ...
    # torch.unique have some bug, which is whether you use sorted or not, it sorted! so bad it are.
    # batches_weight = torch.unique(img_scores,False)
    # so we in here to get True torch.unique
    ...
    # Get anchors with best iou, this is useful compute which anchor to which target
